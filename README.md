...
## Still placeholder — fill in before going live

- Social links (Instagram / LinkedIn / X) in the footer currently point to `#`
  — swap in your real profile URLs.
- Job listings on `careers.html` are illustrative based on your stated
  services — edit titles/descriptions to match your actual openings before
  publishing, so they're accurate.
- `sitemap.xml` / meta `og:url` tags assume the domain
  `https://www.bharatinfotechs.com/` — update if the real domain differs.

## Deploying

Any static host works — no server, database, or build step required:
- **Netlify / Vercel**: drag-and-drop this folder, or connect it as a Git repo.
- **GitHub Pages**: push the folder to a repo and enable Pages.
- **Your own hosting**: upload via FTP to the web root (or a subfolder — the
  relative links handle either case).
